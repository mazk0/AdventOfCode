    ...
    public static long Run(string dataFilepath)
    {
        ...
        for (var i = 0; i < map.Count; i++)
        {
            ...
            for (var j = i + 1; j < map.Count; j++)
            {
                ...
                long steps = Math.Abs(p1.column - p2.column) + Math.Abs(p1.row - p2.row);
                steps += blankRows.Count(x => p1.row < x && x < p2.row || p2.row < x && x < p1.row) * 999_999L;
                steps += blankCols.Count(x => p1.column < x && x < p2.column || p2.column < x && x < p1.column) * 999_999L;
