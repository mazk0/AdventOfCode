...
public static class SevenOne
{
    public static int Run(string dataFilepath)
    {
        var hands = File.ReadLines(dataFilepath).Select(dataRow => dataRow.Split(' ')).Select(parts => new HandOne {Cards = parts[0], Wins = int.Parse(parts[1])}).ToList();
        var orderedHands = hands.OrderBy(x => x.Cards, new CardsComparerOne()).ToList();
...
public class CardsComparerOne : IComparer<string>
{
